(function() {
  ...
  function Scribble(_p) {
    ...
    this.getOffset = function( minVal, maxVal ) {
      return this.roughness*(p.random()*(maxVal-minVal)+minVal);
    }
    ...
    this.buildEllipse = function( cx, cy, rx, ry, offset, overlap ) {
      ...
      p.beginShape();
      p.curveVertex( this.getOffset( -offset, offset )+cx+0.9*rx*Math.cos( radialOffset-this.ellipseInc ),
          this.getOffset( -offset, offset )+cy+0.9*ry*p.sin( radialOffset-this.ellipseInc ) );

      for ( var theta = radialOffset; theta < Math.PI*2+radialOffset-0.01; theta+=this.ellipseInc ) {
        p.curveVertex( this.getOffset( -offset, offset )+cx+rx*Math.cos( theta ),
            this.getOffset( -offset, offset )+cy+ry*p.sin( theta ) );
      }

      p.curveVertex( this.getOffset( -offset, offset )+cx+rx*Math.cos( radialOffset+Math.PI*2+overlap*0.5 ),
          this.getOffset( -offset, offset )+cy+ry*p.sin( radialOffset+Math.PI*2+overlap*0.5 ) );

      p.curveVertex( this.getOffset( -offset, offset )+cx+0.98*rx*Math.cos( radialOffset+overlap ),
          this.getOffset( -offset, offset )+cy+0.98*ry*p.sin( radialOffset+overlap ) );

      p.curveVertex( this.getOffset( -offset, offset )+cx+0.9*rx*Math.cos( radialOffset+overlap*0.5 ),
          this.getOffset( -offset, offset )+cy+0.9*ry*p.sin( radialOffset+overlap*0.5 ) );
      p.endShape();
    }
    ...
    this.scribbleLine = function( x1, y1, x2, y2 ) {
      ...
      var divergePoint = 0.2 + p.random()*0.2;
      ...
      p.beginShape();
      p.vertex(     x1 + this.getOffset( -offset, offset ), y1 + this.getOffset( -offset, offset ) );
      p.curveVertex(x1 + this.getOffset( -offset, offset ), y1 + this.getOffset( -offset, offset ) );
      p.curveVertex(midDispX+x1+(x2 -x1)*divergePoint + this.getOffset( -offset, offset ), midDispY+y1 + (y2-y1)*divergePoint + this.getOffset( -offset, offset ) );
      p.curveVertex(midDispX+x1+2*(x2-x1)*divergePoint + this.getOffset( -offset, offset ), midDispY+y1+ 2*(y2-y1)*divergePoint + this.getOffset( -offset,offset ) );
      p.curveVertex(x2 + this.getOffset( -offset, offset ), y2 + this.getOffset( -offset, offset ) );
      p.vertex(     x2 + this.getOffset( -offset, offset ), y2 + this.getOffset( -offset, offset ) );
      p.endShape();

      p.beginShape();
      p.vertex(     x1 + this.getOffset( -halfOffset, halfOffset ), y1 + this.getOffset( -halfOffset, halfOffset ) );
      p.curveVertex(x1 + this.getOffset( -halfOffset, halfOffset ), y1 + this.getOffset( -halfOffset, halfOffset ) );
      p.curveVertex(midDispX+x1+(x2 -x1)*divergePoint + this.getOffset( -halfOffset, halfOffset ), midDispY+y1 + (y2-y1)*divergePoint + this.getOffset( -halfOffset, halfOffset ) );
      p.curveVertex(midDispX+x1+2*(x2-x1)*divergePoint + this.getOffset( -halfOffset, halfOffset ), midDispY+y1+ 2*(y2-y1)*divergePoint + this.getOffset( -halfOffset, halfOffset ) );
      p.curveVertex(x2 + this.getOffset( -halfOffset, halfOffset ), y2 + this.getOffset( -halfOffset, halfOffset ) );
      p.vertex(     x2 + this.getOffset( -halfOffset, halfOffset ), y2 + this.getOffset( -halfOffset, halfOffset ) );
      p.endShape();
    }

    this.scribbleCurve = function( x1, y1, x2, y2, x3, y3, x4, y4 ) {
      p.bezier(  x1+this.getOffset( -2, 2 ), y1+this.getOffset( -2, 2 ),
    ...
    this.scribbleFilling = function( xCoords, yCoords, gap, angle ) {
      ...
      var hachureAngle = p.radians( angle%180 );
      var cosAngle = Math.cos( hachureAngle );
      var sinAngle = p.sin( hachureAngle );
